...
def cli(x_particles: int, y_particles: int, array_type: str, tensor_type: str, simulation_steps: int, spring_constant: float, driving_force: float, driving_force_modulation_strength: float) -> None:
    ...
    array = array_map[array_type].create(
        nx=x_particles, ny=y_particles, constants=constants, random_start_seed=1
    )
    ...
    simulation = Simulation.create(
        ...
        hydrodynamic_displacements=[
            ...
            BrownianDisplacement.create(
                constants=constants, fluid_dynamics_tensor=tensor
            ),
        ]
    )
